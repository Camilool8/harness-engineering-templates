...
#     test failure (see the practicing-tdd skill / CLAUDE.md).
#   - The marker is consumed (deleted) once an implementation edit is allowed,
#     so one observed failure unlocks one green step.
#
# This is a deliberately simple, zero-dependency stand-in. For a production-grade
# guard that parses real test-runner output and also blocks over-implementation
# and multi-test-at-once, use https://github.com/nizos/tdd-guard instead.
#
