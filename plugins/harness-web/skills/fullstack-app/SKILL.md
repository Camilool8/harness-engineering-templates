...
description: Conventions for a single deployable that owns both frontend and backend. Use when .claude/HARNESS.toml selects web/fullstack-app, or when building a Next.js App Router / SvelteKit / Nuxt / Remix app where Server Actions are the typed boundary, with expand-contract migrations, server-side auth, and the client/server component split.
---
