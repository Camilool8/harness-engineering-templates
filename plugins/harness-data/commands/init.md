...
1. Ask which sub-domain fits, presenting these options (one line each from their SUBDOMAIN.md adopt-if):
   ...
   - **data-analyst-notebook** — ad-hoc and exploratory analysis where the deliverable is a reactive, reproducible notebook that reads from a warehouse and produces charts, tables, or memos with sample-then-scale on every query.
